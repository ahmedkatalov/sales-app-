...
type aiWarehouseParseResult struct {
	...
	AssumedWeight    bool     `json:"assumedWeight"` // вес/объём взят как средний (не указан в накладной)
...
- РАСКРЫВАЙ СОКРАЩЕНИЯ до полного названия: люди пишут коротко. "стакан б" -> "стакан большой", "стакан м" -> "стакан маленький", "стакан ср" -> "стакан средний". Буквы размера: б=большой, м=маленький, ср/с=средний, xl/l=большой, s=маленький. Также "мол"->"молоко", "сах"->"сахар", "кар"->"карамель", "шок"->"шоколад". Сам догадайся по смыслу и верни полное человеческое название.
...
- Если не хватает КОЛИЧЕСТВА или ЦЕНЫ — НЕ СОХРАНЯЙ МОЛЧА, добавь вопрос в questions.
- Размер/вес одной штуки, если он НЕ указан и у похожего товара его нет — НЕ спрашивай: возьми ТИПИЧНЫЙ средний из общих знаний (basePerUnit + unit g/ml, purchaseUnit="pcs"), пометь assumedWeight=true и assumedNote (напр. "≈350 г/шт, среднее"). Молоко/сок/вода пачка/бутылка ≈ 1000 мл; капуста кочан ≈ 350 г; хлеб ≈ 400 г; лук/картофель/яблоко шт ≈ 150 г; масло пачка ≈ 180 г. Так "молоко 5шт" => 5000 мл; "капуста 2шт" => 700 г.
...
- ПРОВЕРЯЙ ЗДРАВЫЙ СМЫСЛ цены и количества. Если количество НЕ указано, а цена слишком большая, чтобы это была ОДНА штука дешёвого товара (спички, соль, салфетки, трубочки, зубочистки, пакеты, специи, сахар) — НЕ считай что купили 1 шт. Прикинь цену за единицу: если она абсурдна (1 спичка/пачка спичек за 200р), значит это ОБЩАЯ сумма за много штук/упаковок — задай вопрос "Сколько ... купили за <цена>?" и НЕ сохраняй молча. Так "спички 200р" => спроси сколько пачек. То же для дорогих товаров, если цена подозрительно мала для указанного количества.
...
6. Если размер НЕ указан:
   ...
   - иначе возьми ТИПИЧНЫЙ средний размер из общих знаний (молоко/сок пачка ≈1000мл, рис пачка ≈900г, масло пачка ≈180г, капуста кочан ≈350г и т.п.), заполни basePerUnit+unit, и пометь assumedWeight=true с assumedNote. НЕ задавай вопрос про размер для обычных продуктов — пользователь сам уточнит при подтверждении, если надо.
...
func aiTextHasExplicitPrice(text string) bool {
	...
	patterns := []string{
		...
		// `р\b` не работает: \b — ASCII-граница, а кириллическая «р» не ASCII-слово,
		// поэтому «900р», «1500р» не распознавались. Ловим «р» перед концом/не-буквой.
		`\d+(?:\.\d+)?\s*(₽|руб|р(?:$|[^а-яёa-z0-9]))`,
	}
	for _, pattern := range patterns {
		if regexp.MustCompile(pattern).MatchString(t) {
			return true
...
func cleanAIProductName(name string) string {
	...
	re := regexp.MustCompile(`\d+(?:[\.,]\d+)?\s*(короб\w*|упак\w*|пач\w*|бутыл\w*|кг|килограмм\w*|гр|грамм\w*|мл|миллилитр\w*|л(?:$|[^а-яёa-z0-9])|литр\w*|шт|штук\w*|шту\w*|₽|руб\w*|р(?:$|[^а-яёa-z0-9]))`)
...
- name: ПРАВИЛЬНОЕ каноничное название БЕЗ слов купил/взял/за/рублей/кг/новые/это. Исправляй опечатки и приводи бренд к корректному написанию: "морожжено максибон"->"Мороженое Maxibon", "кокакола"->"Кока-Кола", "малако"->"Молоко". РАСКРЫВАЙ СОКРАЩЕНИЯ до полного слова: "стакан б"->"стакан большой", "стакан м"->"стакан маленький", "стакан ср"->"стакан средний" (б=большой, м=маленький, ср/с=средний); "мол"->"молоко", "сах"->"сахар". Это нужно, чтобы название совпадало с составом блюд и связка работала.
...
- Если есть и цена, и количество → questions=[]
- ЗДРАВЫЙ СМЫСЛ: если количество НЕ указано, а цена велика для 1 штуки дешёвого товара (спички, соль, салфетки, трубочки, пакеты, специи, сахар) — НЕ ставь purchaseQuantity=1, а спроси количество: questions=["Сколько ... купили за <цена>?"]. Пример: "спички 200р" → спроси сколько пачек. Абсурдная цена за единицу = это общая сумма за много штук.
